"""
Henon data generation (step 1)

Generates clean Hénon x[n], creates noisy variants, saves signals (.npz) + metadata (csv),
...
def stable_hash_int(s: str, modulo: int = 10_000) -> int:
    """
    Stable integer hash for strings (cross-process), avoids Python's randomized hash().
    """
    h = hashlib.md5(s.encode("utf-8")).hexdigest()
    return int(h[:8], 16) % int(modulo)
...
class HenonConfig:
    """
    Configuration container for Hénon data generation.
    ...
    dt:
        Synthetic time step used to create t_eval (also used by low_freq sinusoid).
    warmup_time:
        Transient duration (seconds) that is iterated but removed.
        warmup_samples = int(warmup_time / dt)
    n_runs:
    ...
    a0, b0:
        Base parameters.
    param_jitter:
        Relative jitter applied per run.
    diverge_abs:
        Absolute value threshold to declare divergence.
    max_tries:
        Retry attempts if diverges.
    """

    # signal generation (Lorenz-style: duration/dt + warmup)
    ...
    # base output folder
    base_folder: str = "Henon_Noise_Exp"
    ...
    a0: float = 1.4
    b0: float = 0.3
    param_jitter: float = 0.03

    # robustness
    diverge_abs: float = 1e6
    max_tries: int = 50
...
# Hénon map generator
# -----------------------------
def generate_henon_series(cfg: HenonConfig, run_seed: int) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """
    Generate one clean Hénon x[n] series.
    - Iterate warmup + n_samples steps, then discard warmup.
    - Randomize parameters and initial conditions per run_seed.
    - Return fixed-length arrays (int(duration/dt)).
    """
    reseed_all(run_seed)

    n_samples = int(cfg.duration / cfg.dt)
    warmup_samples = int(cfg.warmup_time / cfg.dt)
    total_len = warmup_samples + n_samples

    for attempt in range(cfg.max_tries):
        # deterministic per (run_seed, attempt)
        reseed_all(int(run_seed) + 1000 * int(attempt))

        a = cfg.a0 * (1.0 + np.random.uniform(-cfg.param_jitter, cfg.param_jitter))
        b = cfg.b0 * (1.0 + np.random.uniform(-cfg.param_jitter, cfg.param_jitter))
        ...
        for i in range(total_len):
            ...
            if (
                ...
                or (abs(x) > cfg.diverge_abs)
                or (abs(y) > cfg.diverge_abs)
            ):
                diverged = True
                break
        ...
        if not (np.isfinite(series).all() and len(series) == n_samples):
            continue

        t_eval = np.arange(len(series), dtype=np.float64) * float(cfg.dt)
        ...
        meta = {
            ...
            "a0": float(cfg.a0),
            "b0": float(cfg.b0),
            "param_jitter": float(cfg.param_jitter),
        ...
        return t_eval.astype(np.float64), series.astype(np.float64), meta

    raise RuntimeError(
        f"Henon generation diverged for run_seed={run_seed} after {cfg.max_tries} attempts. "
...
def plot_example_clean_only(t_eval: np.ndarray, clean: np.ndarray) -> plt.Figure:
    ...
    plt.title("Clean Henon System (no noise)")
    plt.xlabel("Time [s]")
    plt.ylabel("x[n]")
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def generate_henon_dataset(cfg: HenonConfig) -> str:
    ...
    root = make_run_root(cfg.base_folder, tag="henon_data")
    ...
    for run_id in range(1, cfg.n_runs + 1):
        ...
        reseed_all(run_seed)
        t_eval, clean, gen_meta = generate_henon_series(cfg, run_seed=run_seed)
        ...
        if run_id == 1:
            ...
            out_base = os.path.join(paths["fig_signal_examples"], f"Henon_clean_example_run{run_id:04d}")
        ...
        for noise_type, levels in cfg.noise_configs.items():
            for intensity in levels:
                ...
                config_seed = int(
                    1_000_000 * run_id
                    + 10_000 * stable_hash_int(str(noise_type), modulo=100)
                    + int(noise_intensity * 1000)
                )
                ...
                if run_id == 1:
                    ...
                    fig = plot_example_clean_vs_noisy(t_eval, clean_norm2, noisy, noise_type, noise_intensity)
                    out_base = os.path.join(
                        paths["fig_signal_examples"],
                        f"Henon_{noise_type}_intensity{intensity}_run{run_id:04d}",
                    )
                    save_png_eps(fig, out_base)
                    plt.close(fig)
